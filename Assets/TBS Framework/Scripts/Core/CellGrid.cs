...
public class CellGrid : MonoBehaviour
{
	...
	// Newly Added
	/// <summary>
	/// UnitAdded event is invoked each time a unit is moved.
	/// </summary>
	...
	{
		//TODO: command AI player to take action
		//

		//TODO: Find units that changed
		// and move them
		// Units.FindAll
		// Units.FindAll(u => u.PlayerNumber.Equals(CurrentPlayerNumber)).ForEach(u => { u.Move() });
	...
	// Newly Added
	public void MoveUnit(String fromCell, String toCell) {
		List<Unit> units = UnitsForCell (fromCell);
		Cell destination = CellWithLocation (toCell);

		if (units != null && destination != null) {
			units.ForEach (u => {
				List<Cell> path = u.FindPath(Cells, destination);
				u.Move (destination, path);
	...
	// Newly Added
	public Cell CellWithLocation(String location) {
		return Cells.FindLast (c => { return c.Location == location; });
	}

	// Newly Added
	public List<Unit> UnitsForCell(String location) {
		Cell cell = CellWithLocation (location);
		if (cell != null) {
			List<Unit> units = Units.FindAll (u => { return u.Cell == cell; });
			return units;
		}
		return null;
	}
